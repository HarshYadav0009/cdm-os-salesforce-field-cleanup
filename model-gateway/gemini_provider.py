...
class GeminiFreeProvider:
    ...
    async def generate_completion(
        ...
        """
        # Strip provider prefix if given (e.g. gemini/gemini-1.5-flash -> gemini-1.5-flash)
        model_name = model.split("/")[-1] if "/" in model else model

        logger.info(f"Dispatching Gemini request: model={model_name}, configured={self._is_configured}")

        if self._is_configured:
            # Attempt live Google Generative AI SDK call or REST API request if available
            try:
                ...
                input_tokens = len(prompt.split()) * 2  # Estimate if count not returned
        ...
        # Standalone / Development offline simulation payload
        sample_output = (
